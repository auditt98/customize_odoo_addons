...
from . import data_default as data_default
...
class ProductTemplate(models.Model):
    ...
    name = fields.Char('Tên sản phẩm', index=True, required=True, translate=True)
    # customized
    thuong_hieu = fields.Selection(data_default.d_thuong_hieu, string='Thương hiệu')
    loai_da_chinh = fields.Selection(data_default.d_loai_da_chinh, string='Loại đá chính')
    mau_da_chinh = fields.Selection(data_default.d_mau_da_chinh, string='Màu đá chính')
    hinh_dang_da = fields.Selection(data_default.d_hinh_dang_da, string='Hình dáng đá')
    gioi_tinh = fields.Selection(data_default.d_gioi_tinh, string='Giới tính')
    dip_tang_qua = fields.Many2many('product.diptangqua', string='Dịp tặng quà')
    doi_tuong_tang = fields.Many2many('product.doituongtang', string='Quà tặng cho người thân')
    loai_da_phu = fields.Selection(data_default.d_loai_da_phu, string='Loại đá phụ (nếu có)')
    trong_luong_vang = fields.Float(string='Trọng lượng vàng tham khảo (phân vàng)')
    kich_thuoc_da = fields.Float(string='Kích thước đá (ly)', digits=(4,2))
    loai_ngoc_trai = fields.Selection(data_default.d_loai_ngoc_trai, string='Loại ngọc trai')
    mo_ta = fields.Html("Thông tin sản phẩm")
    tuoi_vang = fields.Selection(data_default.d_tuoi_vang, string="Tuổi vàng")
    # test_field = fields.Date("Chọn ngày")
    # test_field = fields.Selection(data_default.d_test, string="Test field")
    # end customized
    sequence = fields.Integer('Sequence', default=1, help='Gives the sequence order when displaying a product list')
    description = fields.Text(
        'Mô tả sản phẩm', translate=True)
    ...
    type = fields.Selection([
        ('consu', 'Consumable'),
        ('service', 'Service')], string='Hình thức sản phẩm', default='consu', required=True,
    ...
    currency_id = fields.Many2one(
        'res.currency', 'Mệnh giá', compute='_compute_currency_id')
    ...
    price = fields.Float(
        'Giá', compute='_compute_template_price', inverse='_set_template_price',
    ...
    standard_price = fields.Float(
        'Giá mua', compute='_compute_standard_price',
    ...
    volume = fields.Float(
        'Thể tích', compute='_compute_volume', inverse='_set_volume', digits='Volume', store=True)
    volume_uom_name = fields.Char(string='Volume unit of measure label', compute='_compute_volume_uom_name')
    weight = fields.Float(
        'Cân nặng', compute='_compute_weight', digits='Stock Weight',
        inverse='_set_weight', store=True)
    weight_uom_name = fields.Char(string='Weight unit of measure label', compute='_compute_weight_uom_name')

    sale_ok = fields.Boolean('Mặt hàng bán được', default=True)
    purchase_ok = fields.Boolean('Mặt hàng mua được', default=True)
    ...
    default_code = fields.Char(
        'Ghi chú', compute='_compute_default_code',
    ...
    #     for record in self:
    #         if record.test_field != '1':
    #             raise ValidationError("Bạn phải chọn giá trị 1")
