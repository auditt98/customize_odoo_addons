...
tien = ["", " ngàn", " triệu", " tỷ", " ngàn tỷ", " triệu tỷ"]


def doc_so_3_chu_so(baso):
	tram = 0
	chuc = 0
	donvi = 0
	ketqua = ""
...
def doctienbangchu(sotien):
	...
	if math.isnan(vitri[5]):
		vitri[5] = "0"
	so = so - float(str(vitri[5])) * 1000000000000000
	vitri[4] = math.floor(so / 1000000000000)
	if math.isnan(vitri[4]):
		vitri[4] = "0"
	so = so - float(str(vitri[4])) * 1000000000000
	vitri[3] = math.floor(so / 1000000000)
	if math.isnan(vitri[3]):
		vitri[3] = "0"
	so = so - float(str(vitri[3])) * 1000000000
	...
	if vitri[5] > 0:
		...
		lan = 0
	for i in range(lan, 0, -1):
		tmp = doc_so_3_chu_so(vitri[i])
		ketqua += tmp
		if vitri[i] > 0:
			ketqua += tien[i]
	...
	ketqua = ketqua[1 : 2].upper() + ketqua[2:] + " đồng."
	return ketqua
...
class AccountMove(models.Model):
    ...
    hinh_thuc_thanh_toan = fields.Selection([('TM', 'Tiền mặt'), ('CK', 'Chuyển khoản')], string="Hình thức thanh toán")
    ky_hieu = fields.Char(string="Ký hiệu", default="AA/20E")
    tien_bang_chu = fields.Char(string="Số tiền viết bằng chữ")
    date = fields.Date(
        string='Date',
        required=True,
        index=True,
    ...
    def _compute_amount(self):
        for move in self:
            ...
            move.amount_total = sign * (total_currency if len(currencies) == 1 else total) * 5
            move.amount_residual = -sign * (total_residual_currency if len(currencies) == 1 else total_residual)
            move.amount_untaxed_signed = -total_untaxed
            move.amount_tax_signed = -total_tax
            move.amount_total_signed = abs(total) if move.move_type == 'entry' else -total
            move.amount_residual_signed = total_residual
            move.tien_bang_chu = doctienbangchu(move.amount_total) #edit
